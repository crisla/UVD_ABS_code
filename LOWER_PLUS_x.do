/// L O W E R +  D O  F I L E //

by id: egen mu_i = mean(Bdays)
by id: replace mu_i = . if nuc2==1
...
by id : gen sig_i = ((Bdays[1]-Bdays[2])^2)/2 if nuc2==2

*-*-*-* sigma y bar hat *-*-*-*

by id : gen sigyi = Bdays[1]^2 + Bdays[2]^2 - 2*mu_y^2 if mu_i!=.
...
gen lBdays = log(Bdays)

*-*-*-* mu hat i *-*-*-*

by id : gen lmu_i = (lBdays[1]+lBdays[2])/2 
by id : replace lmu_i = . if nuc2==1
...
by id : gen lsig_i = ((lBdays[1]-lBdays[2])^2)/2
replace lsig_i=. if nuc2==1

*-*-*-* sigma y bar hat *-*-*-*

by id: gen lsigyi = lBdays[1]^2 + lBdays[2]^2 - 2*lmu_y^2 if lmu_i!=.
...
gen spell_d=1 if nuc2==2
replace spell_d = 0 if nuc2==1

gen age2 = age^2

reg lBdays sex_d i.educ2 spell_d age age2
...
by id : gen lmu_i2 = (lBdays[1]+lBdays[2])/2
by id : replace lmu_i2 = . if [_n]!= 2

egen mu_y2 = mean(lmu_i)

by id : gen lsy = ((lBdays[1]-mu_y2)^2 + (lBdays[2]-mu_y2)^2)/(2*N-1)
